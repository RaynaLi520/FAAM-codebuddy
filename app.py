...
def index():
    ...
    today_new = conn.execute('''
        SELECT dna.id, dna.tcin, dna.title, dna.brand, dna.price,
               COALESCE(p.image_url, dna.image_url) as image_url,
               COALESCE(p.product_url, dna.product_url) as product_url,
               dna.date_detected
...
def products():
    ...
    # Support multiple item_type selection (comma-separated or multiple params)
    item_types_param = request.args.getlist('item_type')
    selected_item_types = [it for it in item_types_param if it]  # Filter empty
    ...
    # Multiple item_type selection
    if selected_item_types:
        placeholders = ','.join(['?'] * len(selected_item_types))
        query += f' AND item_type IN ({placeholders})'
        count_query += f' AND item_type IN ({placeholders})'
        params.extend(selected_item_types)
...
def new_arrivals():
    ...
    # Use COALESCE to prefer products.image_url over daily_new_arrivals.image_url
    if date and date != 'all':
        new_products = conn.execute('''
            SELECT dna.id, dna.tcin, dna.title, dna.brand, dna.price,
                   COALESCE(p.image_url, dna.image_url) as image_url,
                   COALESCE(p.product_url, dna.product_url) as product_url,
                   dna.date_detected, dna.is_processed,
                   p.item_type, p.color_summary, p.color, p.size_summary,
                   p.rating, p.review_count, p.has_promotion, p.discount_percentage,
                   p.is_new, p.retail_price, p.original_price
            FROM daily_new_arrivals dna
            LEFT JOIN products p ON dna.tcin = p.tcin
            WHERE dna.date_detected = ?
